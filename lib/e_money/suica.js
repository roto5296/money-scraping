...
class suica {
  constructor ({ id, password, options }) {
    ...
    const cookieJar = request.jar(new fileCookieStore(this.options.cookie_auth, this.options.cookie_id || process.env.SUICA_COOKIE));
  ...
  async waitInit () {
    if (!this._cookieJar.getCookies("https://www.mobilesuica.com").length) {
