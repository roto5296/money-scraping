...
class rakuten {
  ...
  async getDetails(year, month){
    ...
    for (let checkPage of checkPages) {
      ...
      var query;
      if (checkPage === -1) {
        query = '.stmt-next-payment-list-head .stmt-payment-lists__tbl';
      } else {
        query = '.stmt-current-payment-list-head .stmt-payment-lists__tbl'
      }
      $(query).children().each(function(index){
      ...
      if (checkPage === -1) {
        query = '.stmt-next-payment-list-body .stmt-payment-lists__tbl';
      } else {
        query = '.stmt-current-payment-list-body .stmt-payment-lists__tbl'
      }
      $(query).each(function(){
