...
class rakuten {
  ...
  async getDetails(year, month){
    ...
    for (let checkPage of checkPages) {
      ...
      var detailIndex;
      if (checkPage) {
        $('#pastSortForm\\:listPast thead th').each(function(index){
        ...
        $('#pastSortForm\\:listPast tbody').children().each(function(){
          ...
          const date = columns.eq(dateIndex).text();
          if (parseInt(date.replace(/^.*?\/|\/.*?$/g, ""), 10)
              === month) {
            output.push({
              date: date,
              price: -1 * parseInt(columns.eq(priceIndex).text().replace(/,/g, ""), 10),
              detail: columns.eq(detailIndex).text().replace(/ *$/g, "")
            });
          }
        });
      } else {
        $('#nextLatestSortForm\\:nextListLatest thead th').each(function(index){
          if ($(this).text().match(/利用日/g)) {
            dateIndex = index;
          }
          if ($(this).text().match(/金額/g)) {
        ...
        $('#latestSortForm\\:listLatest thead th').each(function(index){
          if ($(this).text().match(/利用日/g)) {
            dateIndex = index;
          }
          if ($(this).text().match(/金額/g)) {
